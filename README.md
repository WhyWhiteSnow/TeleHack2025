...
При неудачном считывании полей или в случае их отсутствия в документе поля в JSON заполняются пустыми строками.

## Используемый стэк
Все приложение разрабатывалось на python с использованием библиотек для разработки отдельных сервисов, пакетного менеджера uv и pydantic-settings для удобной работы с конфигами. Для контейнеризации использовался Docker.
### Телеграм-бот
Написан на Aiogram, позволяет отправлять асинхронные запросы серверу. Для работы с сервером используется aiohttp. Loguru позволяет локально сохранять логи о работе с пользователем и сервером.
### OCR-сканер
Написан на PyTesseract. CV-модуль, считывающий содержимое документа. API написано на FastAPI, имеет два роута /upload для обработки pdf-документов и /upload-image для обработки изображений.
